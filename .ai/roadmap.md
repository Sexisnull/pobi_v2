...
- [x] **前端创建任务去预检门禁（2026-09-09）**：auto 模式创建任务不再强制「验证凭据」，改为用户填账号密码直接创建、凭据随请求传递，后端 `run_auto_auth` 照常后台自动登录落会话（失败记 `auth_status=failed` 不阻断）。`Tasks.jsx` 移除验证状态机（`verifyState/resetVerify/setAuthField/verify`）、「验证凭据」按钮与结果徽标、submit 凭据验证门禁，认证字段 onChange 统一走 `set`。`POST /api/v1/tasks/verify-auth` 与 `engine/preauth.verify_credentials` 保留但前端不再调用（可按需清理，见 `constraints.md` verify-auth 条目变更标注）。
- [x] **前置认证改 LLM 驱动（2026-09-09，方案 B）**：`run_auto_auth` 的固定 `auth_flow="form"` + 固定 JS 自动填表分支替换为 LLM 驱动前置认证。新增 `observe_login_surface` 工具（`pobi_agent/tools/browser/observe_login_surface.py`：HTTP 探测 `WWW-Authenticate` → HTTP Basic 特征；BrowserSession 导航提取表单 input/button/form 字段、OAuth 跨域重定向、body 摘要，输出 secret-free）+ `PreAuthAgent`（`pobi_agent/agents/generic_agents/preauth_agent.py`：工具集 observe+authenticate+validate+refresh，deps 用轻量 `PreAuthDeps`，指令模板 `pobi_prompts/preauth.instructions.jinja2`，凭据走任务钱包 `profile=preauth`、明文不进 LLM 上下文）；`run_auto_auth` 改为构造 `PreAuthAgent`（`get_model_spec`）运行，整体 180s 超时，判定优先看 `tasks/<id>/agent/auth_context/preauth.playwright.json` 落盘证据，LLM 摘要带 MFA 标记 → `auth_status=mfa`，异常/超时 → `failed` 不阻断。`verify_credentials`（verify-auth 预检）与 `authenticate_service` 未改（前端已不调用）。测试 `tests/test_preauth.py` 重写 run_auto_auth 用例（mock PreAuthAgent）+ 新增 observe_login_surface 3 项，28 passed；认证相关回归（test_auth/test_pre_recon）40 passed；ruff 全绿。
